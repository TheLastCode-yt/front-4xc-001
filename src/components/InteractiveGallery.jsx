    ...
    <>
      <section className="py-10">
        <div className="border-y-[0.5px] border-gray-200/70">
          {/* mobile version */}
          <div className="flex flex-col gap-y-6 xl:hidden">
            <div className="flex overflow-x-scroll xl:hidden">
              {items.map((item, index) => (
                <button
                  key={`${index}+${item.title}btn-mobile`}
                  className={`p-10  items-center text-sm sm:text-base font-medium border border-[#D9D9D9] ${
                    activateItem.title === item.title
                      ? 'text-black border-t-4 border-t-custom__blue bg-custom__blue-focus'
                      : 'text-custom__blue flex'
                  }`}
            ...
            <div className="container mx-auto text-black flex gap-y-6 flex-col">
              <h3 className="text-4xl">{activateItem.title}</h3>
          ...
          <div className="container mx-auto hidden xl:block">
            <div className="w-full flex h-[520px] relative">
              ...
              <Image
                ...
                width={686}
                height={520}
                className="object-contain max-h-[520px] xl:absolute -z-10 transition-opacity duration-300 top-[60px] bottom-0 right-0"
              />

              <div className="hidden absolute xl:flex items-center pl-[4%] w-full h-full text-black pointer-events-none">
